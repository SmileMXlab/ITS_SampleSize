################################################################################################
### simcheck02.R: script for simulating, analysing data, and sample size estimation          ###
...
GLMmod<-function(dataframe,greatmod){
	...
	mod_GLM<-eval(parse(text=paste("glm(y~",greatmod,",data=dataframe,family=poisson)",sep="") ))
...
GLMpower<-function(nsmp,pre_nsmp,beta,greatmod,sim){
	...
	NSum<-colSums(matrixy)/(sim) 
	pointest<-data.frame(Method="GLM",N=nsmp,beta0=beta[1],beta_t=beta[2],beta_X=beta[3],beta_Xt=beta[4],
		biasX=mean(result$biasX)-beta[3],
		biasXt=mean(result$biasXt)-beta[4],
		AlphaGLMX  =sum(result$PX<0.05)/sim,
		AlphaGLMXt =sum(result$PXt<0.05)/sim,
		before_N=mean(NSum[1:(nsmp/2)]),
		after_N=mean(NSum[(nsmp/2+1):(nsmp)]))
...
Obsmod<-function(dataframe){
	...
	mod_INGARCH<-tsglm(y, model=list(past_obs=1), xreg=Xtdata, link="log", distr="poisson")
      dummy<-ci.lin(mod_INGARCH,subset=c("X"),Exp=T)
...
Obspower<-function(nsmp,pre_nsmp,beta,rho,sim){
	...
	pointest<-data.frame(Method="INGARCH",N=nsmp,beta0=beta[1],beta_t=beta[2],beta_X=beta[3],beta_Xt=beta[4],
		biasX=mean(result$biasX)-beta[3],
		biasXt=mean(result$biasXt)-beta[4],
		AlphaINGARCHX  =sum(result$PX<0.05)/sim,
		AlphaINGARCHXt =sum(result$PXt<0.05)/sim,
		AlphaMCX   =sum( ifelse(( 2*(1-pnorm(abs(result$biasX/sd(result$biasX) )))<0.05 ),1,0) )/sim,
		AlphaMCXt  =sum( ifelse(( 2*(1-pnorm(abs(result$biasXt/sd(result$biasXt) )))<0.05 ),1,0) )/sim,
		before_N=mean(NSum[1:(nsmp/2)]),
		after_N=mean(NSum[(nsmp/2+1):(nsmp)]))
...
Parpower<-function(nsmp,pre_nsmp,beta,rho,sigma_mu2,greatmod,sim){
	...
	pointest<-data.frame(Method="MSRC",N=nsmp,beta0=beta[1],beta_t=beta[2],beta_X=beta[3],beta_Xt=beta[4],rho=rho,sigma_mu2=sigma_mu2,
		biasX=mean(result$biasX)-beta[3],
		biasXt=mean(result$biasXt)-beta[4],
		AlphaMSRCX  =sum(result$PX<0.05)/sim,
		AlphaMSRCXt =sum(result$PXt<0.05)/sim,
		before_N=mean(NSum[1:(nsmp/2)]),
		after_N=mean(NSum[(nsmp/2+1):(nsmp)]))
...
Sys.time()
#GLMpower(nsmp=100,pre_nsmp=50,beta=c(log(2),log(2),0,0),greatmod="time+X+Xt",sim=1000)
GLMN(pold=400,beta=c(log(2),log(2),log(1.5),log(4)),sim=1000,itmax=3,type="X")                             # 218
GLMN(pold=400,beta=c(log(2),log(2),log(1.5),log(4)),sim=1000,itmax=3,type="Xt")                            # 218
Sys.time()


Sys.time()
#Obspower(nsmp=100,pre_nsmp=50,beta=c(log(2),log(2),0,0),rho=0.2,sim=100)
ObsN(pold=400,beta=c(log(2),log(2),log(1.5),log(4)),rho=0.2,sim=100,itmax=3,method="INGARCH",type="X")     # 172
ObsN(pold=400,beta=c(log(2),log(2),log(1.5),log(4)),rho=0.2,sim=100,itmax=3,method="INGARCH",type="Xt")    # 196
ObsN(pold=400,beta=c(log(2),log(2),log(1.5),log(4)),rho=0.2,sim=100,itmax=3,method="MC",type="X")          # 172
ObsN(pold=400,beta=c(log(2),log(2),log(1.5),log(4)),rho=0.2,sim=100,itmax=3,method="MC",type="Xt")         # 172


Sys.time()
#Parpower(nsmp=100,pre_nsmp=50,beta=c(log(2),log(2),0,0),rho=0.2,sigma_mu2=0.5,greatmod="time+X+Xt",sim=50)
ParN(pold=100,beta=c(log(2),log(2),log(1.5),log(4)),rho=0.2,sigma_mu2=0.5,sim=50,itmax=3,type="X")          # 2550
ParN(pold=100,beta=c(log(2),log(2),log(1.5),log(4)),rho=0.2,sigma_mu2=0.5,sim=50,itmax=3,type="Xt")         # 500
Sys.time() 
...
Sys.time()
set.seed(202305)
GLMN(pold=400,beta=c(log(2),log(2),log(1.5),0),sim=1000,itmax=1000,type="X")  #234
Sys.time()
set.seed(202305)
GLMN(pold=400,beta=c(log(2),log(2),0,log(4)),sim=1000,itmax=1000,type="Xt")    #244
Sys.time()
set.seed(202305)
ObsN(pold=400,beta=c(log(2),log(2),log(1.5),0),rho=0.6,sim=1000,itmax=1000,method="INGARCH",type="X")   #30
Sys.time()
set.seed(202305)
ObsN(pold=400,beta=c(log(2),log(2),0,log(4)),rho=0.6,sim=1000,itmax=1000,method="INGARCH",type="Xt")     #42
Sys.time()
ITSsim_Par(nsmp=340,pre_nsmp=170,beta=c(1,1,0,0),rho=0.2,sigma_mu2=0.5,greatmod="time+X+Xt",sim=10000)
Sys.time()